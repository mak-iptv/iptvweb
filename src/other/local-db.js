...
const findAll = (type, fav) => {
  if (fav !== true)
    return history[type].sort((a,b) => a.date - b.date).map(x => ({...x, date: new Date(x.date)}));
  else 
...
const del = (type, key, fav) => {
  if(fav !== true)
    ...
    favorite[type] = favorite[type].filter(x => parseInt(x.id) !== parseInt(key));
...
// ✅ Export me variable për të shmangur gabimin ESLint
const localDb = {findOne, findAll, set, del};
